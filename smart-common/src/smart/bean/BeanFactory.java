			...
			public void run() {
				...
				if (hList != null && hList.size() > 0) {
					for (int i = 0; i < hList.size(); i++) {
						...
						List<CPU> cList = hostDao.getCPUById(hList.get(i)
								.getId());
						...
						if (nList != null && nList.size() > 0) {
							for (int j = 0; j < nList.size(); j++) {
								...
								nif.setDescription(nList.get(j)
										.getDescription());
								nif.setDestination(nList.get(j)
										.getDestination());
								...
								if (nisList != null && nisList.size() > 0) {
									for (int k = 0; k < nisList.size(); k++) {
										...
										nis.setStatus(nisList.get(k)
												.getStatus());
										nis.setThroughput(nisList.get(k)
												.getThroughput());
										nis.setTraffic(nisList.get(k)
												.getTraffic());
										nis.setTxBytes(nisList.get(k)
												.getTxBytes());
						...
						List<FileSystem> fList = hostDao
								.getFileSystemById(hList.get(i).getId());
						...
						List<Progress> pList = hostDao.getProgressById(hList
								.get(i).getId());
						if (pList != null && pList.size() > 0) {
							for (int j = 0; j < pList.size(); j++) {
								Progress pro = new Progress(pList.get(j)
										.getId());
								pro.setName(pList.get(j).getName());
								pro.setStartTime(pList.get(j).getStartTime());
								pro.setUser(pList.get(j).getUser());

								List<ProgressDetection> pdList = hostDao
										.getProgressDetecById(pro.getId());
								if (pdList != null && pdList.size() > 0) {
									for (int k = 0; k < pdList.size(); k++) {
										ProgressDetection pd = new ProgressDetection(
												pdList.get(k).getTimestamp());
										pd.setCpuTime(pdList.get(k)
												.getCpuTime());
										pd.setCpuUsed(pdList.get(k)
												.getCpuUsed());
										pd.setMemShare(pdList.get(k)
												.getMemShare());
										pd.setMemSize(pdList.get(k)
												.getMemSize());
										pd.setMemUsed(pdList.get(k)
												.getMemUsed());
										pd.setTimestamp(pdList.get(k)
												.getTimestamp());
