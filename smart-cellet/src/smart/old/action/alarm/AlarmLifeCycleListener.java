...
public final class AlarmLifeCycleListener extends AbstractListener {
	...
	public void onAction(ActionDialect action) {
		...
		if (token != null && !"".equals(token)) {
			...
			case HttpStatus.OK_200:
				...
				if (null != bytes) {
					...
					try {
						...
						this.response(token, Action.ALARMLIFECYCLE, params);

					} catch (JSONException e) {
						e.printStackTrace();
					}
				} else {
					this.reportHTTPError(token, Action.ALARMLIFECYCLE);
				}
				break;
			default:
				Logger.w(AlarmLifeCycleListener.class,
			...
			params.addProperty(new ObjectProperty("data", jo));
			this.response(Action.ALARMLIFECYCLE, params);
		}
	}
}