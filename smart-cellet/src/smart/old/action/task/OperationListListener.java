...
public class OperationListListener extends AbstractListener{

	public OperationListListener(Cellet cellet) {
		super(cellet);
	}
	...
	public void onAction(ActionDialect action) {
		...
		StringBuilder url = new StringBuilder(context.getAPIHost()).append("/").append(API.LISTOPERATION);
		JSONObject json = null;
		String jbpmTaskInstanceID=null;
		try {
			json = new JSONObject(action.getParamAsString("data"));
			jbpmTaskInstanceID=json.getString("jbpmTaskInstanceID");
		...
		case HttpStatus.OK_200:
			...
			if (null != bytes) {
				...
				try {
					...
					this.response(Action.LISTOPERATION, params);

				} catch (JSONException e) {
					e.printStackTrace();
				}
			} else {
				this.reportHTTPError(Action.LISTOPERATION);
			}
