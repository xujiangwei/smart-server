...
public class MoRouterListener extends AbstractListener {

	public MoRouterListener(Cellet cellet) {
		super(cellet);
	}
		...
		case HttpStatus.OK_200:
			...
			if (null != bytes) {
				...
				try {
					...
					System.out.println("moRouter<---" + data);
					// 设置参数
				...
				this.response(Action.MOROUTERS, params);
			} else {
				this.reportHTTPError(Action.MOROUTERS);
			}
			break;
		default:
			Logger.w(MoRouterListener.class, "返回响应码:" + response.getStatus());
			try {
				data = new JSONObject();
				data.put("status", 900);
			} catch (JSONException e) {
