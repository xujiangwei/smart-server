...
public class MoTopoListener extends AbstractListener {

	public MoTopoListener(Cellet cellet) {
		super(cellet);
	}
		...
		case HttpStatus.OK_200:
			...
			if (null != bytes) {
				...
				try {
					...
					System.out.println("moTopo<---" + data);
					// 设置参数
				...
				this.response(Action.MOTOPOS, params);
			} else {
				this.reportHTTPError(Action.MOTOPOS);
			}
			break;
		default:
			Logger.w(MoTopoListener.class, "返回响应码:" + response.getStatus());
			try {
				data = new JSONObject();
				data.put("status", 900);
			} catch (JSONException e) {
