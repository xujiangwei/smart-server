...
public class UserListListener extends AbstractListener{

	public UserListListener(Cellet cellet) {
		super(cellet);
	}
	...
	public void onAction(ActionDialect action) {
		...
		StringBuilder url = new StringBuilder(context.getAPIHost()).append("/").append(API.COMMONUSERLIST);
		...
		case HttpStatus.OK_200:
			...
			if (null != bytes) {
				...
				try {
					...
					this.response(Action.COMMONUSERLIST, params);

				} catch (JSONException e) {
					e.printStackTrace();
				}
			} else {
				this.reportHTTPError(Action.COMMONUSERLIST);
			}
			break;
		default:
