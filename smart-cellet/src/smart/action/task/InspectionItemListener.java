...
public class InspectionItemListener extends AbstractListener{

	public InspectionItemListener(Cellet cellet) {
		super(cellet);
	}
	...
	public void onAction(ActionDialect action) {
		...
		StringBuilder url = new StringBuilder(context.getAPIHost()).append("/").append(API.INSPECTIONITEMLIST);
		...
		System.out.println("获取巡检项的URL:" + url.toString());
		...
		case HttpStatus.OK_200:
			...
			if (null != bytes) {
				...
				try {
					...
					System.out.println("巡检项数据为:" + jo);

					// 设置参数
					params.addProperty(new ObjectProperty("data", jo));

					// 响应动作，即向客户端发送 ActionDialect
					this.response(Action.INSPECTIONITEMLIST, params);

				} catch (JSONException e) {
					e.printStackTrace();
				}
			} else {
				this.reportHTTPError(Action.INSPECTIONITEMLIST);
			}
